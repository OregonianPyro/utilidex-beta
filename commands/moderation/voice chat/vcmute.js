...
class VCMute extends Command {
    constructor(client) {
        super(client, {
            name: 'vcmute',
            category: 'moderation',
            description: 'Mutes a user in all voice chats easily and quickly.',
            usage: '{prefix}vcmute <@user|user ID> [reason]',
    ...
    async run(message, args) {
        if (!args[0]) return this.client.help(this.client, message, 'vcmute');
        ...
        if (member.serverMute) return this.client.error(message, 'That user is already muted server wide.');
        const reason = args.slice(1).join(' ').length > 0 ? args.slice(1).join(' ') : 'No Reason Provided';
        message.delete();
        try {
            await member.setMute(true, `Muted by ${message.author.tag} | Reason: ${reason}`);
        } catch (e) {
            return console.error, this.client.error(message, `Error: \`${e.message}\``);
        };
        message.channel.send(`${member.toString()} has been \`🔇\` muted in all voice channels by ${message.author.tag}\`\`\`${reason}\`\`\``);
        ...
        const modLog = new RichEmbed()
            .setColor('GREY')
            .setAuthor(`${member.user.tag} | Voice-Chat Mute`, member.user.displayAvatarURL)
            .setDescription(`**${member.user.tag}** (\`${member.user.id}\`) was muted in all voice chats by ${message.author.tag}`)
