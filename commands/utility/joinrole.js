...
class JoinRole extends Command {
    constructor(client) {
        super(client, {
            name: 'joinrole',
            category: 'utility',
            description: 'Gives you a role from the server\'s public-role list.',
    ...
    async run(message, args) {
        if (!args[0]) {
            ...
            const msg = await message.channel.send(`Type the number of what role you would like to join:\`\`\`${array.join('\n')}\`\`\``);
            message.channel.awaitMessages(m => m.author.id === message.author.id, { max: 1, time: 60000, errors: ['time'] }).then(async messages => {
                ...
                if (messages.first().content > 0 && messages.first().content <= number) {
                    ...
                    if (message.member.roles.has(role.id)) return msg.edit(`${this.client.emotes.x} You already have the role \`${role.name}\``);
                    try {
                        await message.member.addRole(role.id, 'Adding Join-Roles');
                    } catch (e) {
                        return msg.edit(`Error: ${e.message}`);
                    };
                    return msg.edit(`${this.client.emotes.check} You now have the role \`${role.name}\``);
                };
            }).catch(() => {
            ...
            if (message.member.roles.has(role.id)) return message.channel.send(`${this.client.emotes.x} You already have the role \`${role.name}\``);
            try {
                await message.member.addRole(role.id, 'Adding Join-Roles');
            } catch (e) {
                return msg.edit(`Error: ${e.message}`);
            };
            return message.channel.send(`${this.client.emotes.check} You now have the role \`${role.name}\``);
        };
    };
};
